...
class ViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Generar  las imagenes con border circulares
...
//        }
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let shareAction = UITableViewRowAction(style: .default, title: "Compartir") { (action, indexPath) in
            ...
            let shareDefaultText = "Estoy mirando la receta de \(self.recipes[indexPath.row].name!) en la APP del curso de IOS 10 de Juan Gabriel"
        ...
        shareAction.backgroundColor = UIColor(red: 30.0/255, green: 164.0/255, blue: 253.0/255, alpha: 1.0)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       /*  let recipe = recipes[indexPath.row]
        ...
        let alertController = UIAlertController(title: recipe.name, message: "Valora este plata", preferredStyle: .alert)
        let cancelAction = UIAlertAction(title: "Cancelar", style: .cancel, handler: nil)
        alertController.addAction(cancelAction)
        
        var favouriteTitle = "Favorito"
        var favouriteActionStyle = UIAlertActionStyle.default
        if recipe.isFavourite{
            favouriteTitle = "No Favorito"
            favouriteActionStyle = UIAlertActionStyle.destructive
        }
        
        let favoriteAction = UIAlertAction(title: favouriteTitle, style: favouriteActionStyle) { (action) in
